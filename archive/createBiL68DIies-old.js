...
function processFile(refPath,path) {
  ...
  var shieldValDir = originalFileName[1]
  var shieldValInd = originalFileName[2]

  const newDataDir = processCSV(refPath,shieldValDir);
  const newDataInd = processCSV(refPath,shieldValInd);
  ...
  Object.keys(newDataDir).forEach((colorD) => {
    Object.keys(newDataInd).forEach((colorI) => {
      if(colorD.substring(0,3)===colorI.substring(0,3)) {
        lengths.forEach((length) => {

          var newFixtureData = originalData.fixtureData.split(' ');
          var newWattageData = originalData.wattageData.split(' ');

          newFixtureData[2] = (((newDataDir[colorD][0] + newDataInd[colorI][0]) * Number(length)) / Number(originalData.absLumen)).toFixed(5);

          newWattageData[2] = ((newDataDir[colorD][1] + newDataInd[colorI][1]) * Number(length)).toFixed(1);
          ...
          var newFile = oldFile.map((val,ind) => {
            if (ind===oldFile.length - 3) {
              return colorI
            } else if (ind === oldFile.length - 2) {
              return colorD
